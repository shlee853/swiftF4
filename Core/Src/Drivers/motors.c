...
{
  ASSERT(id < NBR_OF_MOTORS);
  GPIO_PassthroughInput.Pin = motorMap[id]->gpioPin;
  HAL_GPIO_Init(motorMap[id]->gpioPort, &GPIO_PassthroughInput);
}

void motorsESCSetOutput(uint32_t id)
{
  ASSERT(id < NBR_OF_MOTORS);
  GPIO_PassthroughOutput.Pin = motorMap[id]->gpioPin;
  GPIO_PassthroughOutput.Mode = motorMap[id]->gpioOType;
  HAL_GPIO_Init(motorMap[id]->gpioPort, &GPIO_PassthroughOutput);
}

void motorsESCSetHi(uint32_t id)
{
  ASSERT(id < NBR_OF_MOTORS);
  HAL_GPIO_WritePin(motorMap[id]->gpioPort, motorMap[id]->gpioPin, GPIO_PIN_SET);
}

void motorsESCSetLo(uint32_t id)
{
  ASSERT(id < NBR_OF_MOTORS);
  HAL_GPIO_WritePin(motorMap[id]->gpioPort, motorMap[id]->gpioPin, GPIO_PIN_RESET);
}

int motorsESCIsHi(uint32_t id)
{
  ASSERT(id < NBR_OF_MOTORS);
  return HAL_GPIO_ReadPin(motorMap[id]->gpioPort, motorMap[id]->gpioPin) != GPIO_PIN_RESET;
}
...
//    TIM_TimeBaseStructInit(&TIM_TimeBaseStructure);
    ...
    if (enable)
    {
      TIM_TimeBaseStructure.Prescaler = (5 - 1);
      TIM_TimeBaseStructure.Period = (uint16_t)(MOTORS_TIM_BEEP_CLK_FREQ / frequency);
    }
    else
    {
      TIM_TimeBaseStructure.Period = motorMap[id]->timPeriod;
      TIM_TimeBaseStructure.Prescaler = motorMap[id]->timPrescaler;
    }
